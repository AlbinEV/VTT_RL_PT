# File: polish_kz_dz_control_reward.py
"""
Module: polish_kz_dz_control_reward.py
Brief: Compute the reinforcement learning reward for the polishing task with Kp_Z + damping control.
This reward is designed for RL training where the agent controls Z-axis stiffness (Kp_Z)
and damping ratio (zeta_Z). The reward focuses on:
 1. Optimal force control during contact phase
 2. Force stability and consistency with damping-aware penalties
 3. Adaptive stiffness for different contact conditions
 4. Soft contact shaping with early damping bonuses
 5. Minimal force oscillations in steady contact
...
def _compute_polish_kz_dz_control_reward(env) -> torch.Tensor:
    """
    Compute Kp_Z + damping focused polishing reward for all environments.
    ...
    W_SOFT_DZ_BONUS = float(os.environ.get("W_SOFT_DZ_BONUS", "2.0"))
    SOFT_DZ_TARGET = float(os.environ.get("SOFT_DZ_TARGET", "0.9"))
    SOFT_DZ_SCALE = float(os.environ.get("SOFT_DZ_SCALE", "0.25"))
    W_DZ_DF_PENALTY = float(os.environ.get("W_DZ_DF_PENALTY", "2.0"))
    DZ_DF_SCALE = float(os.environ.get("DZ_DF_SCALE", "6.0"))
    DZ_OSC_TARGET = float(os.environ.get("DZ_OSC_TARGET", "0.9"))
    DZ_OSC_GAIN = float(os.environ.get("DZ_OSC_GAIN", "2.0"))
    W_DZ_CHANGE_PENALTY = float(os.environ.get("W_DZ_CHANGE_PENALTY", "1.0"))
    DZ_CHANGE_SCALE = float(os.environ.get("DZ_CHANGE_SCALE", "0.2"))
    W_DZ_STABLE_PENALTY = float(os.environ.get("W_DZ_STABLE_PENALTY", "2.0"))
    W_DZ_HIGH_PENALTY = float(os.environ.get("W_DZ_HIGH_PENALTY", "3.0"))
    DZ_STABLE_MIN = float(os.environ.get("DZ_STABLE_MIN", "0.6"))
    DZ_STABLE_MAX = float(os.environ.get("DZ_STABLE_MAX", "1.4"))
    DZ_STABLE_SCALE = float(os.environ.get("DZ_STABLE_SCALE", "0.2"))
    ...
    if hasattr(env, "dynamic_zeta"):
        current_dz = env.dynamic_zeta[:, 2]
    else:
        current_dz = torch.full_like(current_kpz, 0.9)
    ...
    # Soft contact: encourage damping ratio in a target band early on
    soft_dz_error = torch.abs(current_dz - SOFT_DZ_TARGET)
    r_soft_dz = mask_contact * (1.0 - contact_ramp) * torch.exp(
        -soft_dz_error / max(SOFT_DZ_SCALE, 1e-6)
    ) * W_SOFT_DZ_BONUS

    # Penalize force oscillations based on damping ratio deviation
    dz_osc_weight = 1.0 + torch.abs(current_dz - DZ_OSC_TARGET) * DZ_OSC_GAIN
    p_dz_df = mask_contact * contact_ramp * (
        (fz_change / max(DZ_DF_SCALE, 1e-6)) * dz_osc_weight
    ) * W_DZ_DF_PENALTY

    # Penalize large damping changes to avoid aggressive swings
    if hasattr(env, "_prev_dz"):
        dz_change = torch.abs(current_dz - env._prev_dz)
        p_dz_change = mask_contact * (dz_change / max(DZ_CHANGE_SCALE, 1e-6)) * W_DZ_CHANGE_PENALTY
        env._prev_dz = current_dz.clone()
    else:
        p_dz_change = torch.zeros(env.num_envs, device=env.device)
        env._prev_dz = current_dz.clone()

    # Penalize damping ratio outside a stable contact range
    dz_low = torch.relu(DZ_STABLE_MIN - current_dz)
    dz_high = torch.relu(current_dz - DZ_STABLE_MAX)
    dz_scale = max(DZ_STABLE_SCALE, 1e-6)
    p_dz_low = mask_contact * contact_ramp * (dz_low / dz_scale) * W_DZ_STABLE_PENALTY
    p_dz_high = mask_contact * contact_ramp * (dz_high / dz_scale) * W_DZ_HIGH_PENALTY
    p_dz_stable = p_dz_low + p_dz_high
    ...
    reward = (
        ...
        r_soft_dz +              # soft contact damping preference
        ...
        - p_dz_df - p_dz_change - p_dz_stable
    )
    ...
    terms = {
        ...
        'p_dz_df': p_dz_df,
        'p_dz_change': p_dz_change,
        'p_dz_low': p_dz_low,
        'p_dz_high': p_dz_high,
        'p_dz_stable': p_dz_stable,
...
compute_polish_kz_dz_control_reward = _compute_polish_kz_dz_control_reward

__all__ = [
    "_compute_polish_kz_dz_control_reward",
