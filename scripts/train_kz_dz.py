#!/usr/bin/env python3
"""
Training script for Kp_z + damping (zeta_z) control.
Uses the same RL-Games setup as Kp_z-only training.
...
parser = argparse.ArgumentParser(description="Train Kp_z + damping policy")
parser.add_argument("--num_envs", type=int, default=16, help="Number of environments")
parser.add_argument("--max_epochs", type=int, default=200, help="Max training epochs")
parser.add_argument("--headless", action="store_true", default=True)
parser.add_argument("--experiment_name", type=str, default="kz_dz", help="Experiment name")
...
# Enable both Kp and damping deltas for Z axis
...
def main():
    print("\n" + "=" * 60)
    print("TRAINING: Kp_z + Damping (zeta_z) CONTROL")
    print("=" * 60)

    # Create environment
    env_cfg = polish_env.PolishEnvCfg()
    env_cfg.scene.num_envs = args.num_envs
    env_cfg.reward_type = "kz_dz"
    env = polish_env.PolishEnv(cfg=env_cfg, render_mode=None)

    print(f"Action space: {env.action_space}")
    print(f"Observation space: {env.observation_space}")

    if env.action_space.shape[0] != 2:
        print(f"[WARNING] Expected action_dim=2, got {env.action_space.shape[0]}")
